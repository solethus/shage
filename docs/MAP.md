...
| `.github/` | `PULL_REQUEST_TEMPLATE.md` (ours); `workflows/` is upstream CI — `ci.yml`/`release.yml` minimally patched for the new layout and `ci.yml`'s Format and Clippy jobs pinned to a Rust version (the comment there says why), `build_nix.yml` unverified; release and self-update plumbing still target agavra/tuicr — do not run `shage update`. |
